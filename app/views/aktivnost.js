...
export function renderMojaAktivnost({ dashboard, expanded = false, chatEnabled = false }) {
  if (!dashboard) return "";

  const expandedClass = expanded ? " aktivnost-section--expanded" : "";
  const stats = `
    <div class="aktivnost-stats">
      <div class="aktivnost-stat"><span class="aktivnost-stat__val">${dashboard.publishedJobs}</span><span class="aktivnost-stat__lbl">Oglasi</span></div>
      <div class="aktivnost-stat"><span class="aktivnost-stat__val">${dashboard.activeApplies}</span><span class="aktivnost-stat__lbl">Na čekanju</span></div>
      <div class="aktivnost-stat"><span class="aktivnost-stat__val">${dashboard.agreedJobs}</span><span class="aktivnost-stat__lbl">U tijeku</span></div>
      <div class="aktivnost-stat"><span class="aktivnost-stat__val">${dashboard.finishedJobs}</span><span class="aktivnost-stat__lbl">Završeno</span></div>
    </div>`;

  const chatRows = dashboard.chatRows
    .map((row) => {
      const chatLink =
      ...
      return `
        <article class="aktivnost-row">
          <div>
            ...
            <p class="aktivnost-row__meta">${row.isWorker ? "Prijavio/la si se" : "Tvoj oglas"}</p>
          </div>
          ${chatLink}
        </article>`;
    })
