...
function normalizedStatus(status) {
  return String(status || "").trim().toLowerCase() === "zauzet" ? "zauzet" : "slobodan";
}

/** Public majstor/kreator chip — Android „Slobodan sada“ / „Zauzet“. */
function renderPublicStatusChip(user) {
  if (!isWorker(user?.role)) return "";
  const status = normalizedStatus(user?.status);
  const isFree = status === "slobodan";
  return `
    <span class="profile-status-chip${isFree ? " profile-status-chip--free" : " profile-status-chip--busy"}">
      ${isFree ? "Slobodan sada" : "Zauzet"}
    </span>`;
}
      ...
      <form id="profile-edit-form" class="stack-form">
        ...
        ${
          ...
          <select class="field" name="status">
            <option value="slobodan" ${normalizedStatus(user.status) === "slobodan" ? "selected" : ""}>Slobodan</option>
            <option value="zauzet" ${normalizedStatus(user.status) === "zauzet" ? "selected" : ""}>Zauzet</option>
          </select>
...
export function renderPregledProfila({
  ...
  const avatarHtml = renderAvatar(user);
  // Android public: hide individual; show business only.
  const representationHtml =
    user.representationType === "business" ? renderRepresentationSummary(user) : "";
  ...
  return `
    <div class="screen-scroll">
      ...
      <article class="profile-card">
        ...
        ${renderProfileMetaBadges(user, { showBusyBadge: false })}
        <p class="profile-card__meta">${role} · ${category}</p>
        <p class="profile-card__meta profile-card__meta--row">
          <span>${city}</span>
          ${worker ? renderPublicStatusChip(user) : ""}
        </p>
        ${representationHtml}
