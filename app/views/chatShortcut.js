...
export function renderChatShortcut({ href, unread = 0, label = "Poruke" }) {
  const badge =
    unread > 0
      ? `<span class="chat-shortcut__badge">${unread > 9 ? "9+" : escapeHtml(String(unread))}</span>`
      : "";
  return `<a class="chat-shortcut" href="${href}" aria-label="Poruke">${CHAT_ICON}<span class="chat-shortcut__label">${escapeHtml(label)}</span>${badge}</a>`;
}
